...
evalFile('publish-desktop.json', ['sideKeyCalculation', 'createMessage', 'attachMessage'], ['mwm'])

function evalFile(name, durationFields = ['duration'], groupingFields = ['format']) {
  try {
    ...
    for (let point of data) {
      const group = groupingFields.map(n => ({[n]: point[n]})).reduce((acc, v) => ({ ...acc, ...v}));
      const key = hash(group);
      if (!map.has(key)) {
        map.set(key, []);
        hashes.set(key, group);
      }
      const stats = durationFields.map(k => ({ [k]: point[k] })).reduce((acc, v) => ({...acc, ...v}));
      map.get(key).push(stats);
    }
    const absolute = new Map();
    Array.from(map.entries()).forEach(([key, value]) => {
      const getPoints = name => value.map(p => p[name]);
      const stats = durationFields.map(k => ({ [k]: {
    ...
    Array.from(absolute.entries()).forEach(([key, value]) => {
      ...
      props.forEach(prop => {
        ...
        groupPortions[prop] = durationFields
          .map(durationName => ({ [durationName]: value[durationName][prop] / sum}))
          .reduce((acc, v) => ({...acc, ...v}));
      });
