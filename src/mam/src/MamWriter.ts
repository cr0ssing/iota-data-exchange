...
export class MamWriter {
  ...
  constructor(
    provider: string,
    seed: string = keyGen(81),
    mode: MAM_MODE,
    sideKey?: string,
    security: MAM_SECURITY = MAM_SECURITY.LEVEL_1
  ) {
    // Set IOTA provider
    ...
    if (!isTrytesOfExactLength(seed, 81)) {
      console.log(
        'ERROR: Invalid Seed has been submitted. The seed has been replaced with a random seed!'
      );
      seed = keyGen(81);
    }
    this.seed = seed;
  ...
  public async createAndAttach(message: string) {
    ...
    const Result2 = await this.attach(Result.payload, Result.address);
    return Result2;
  }
  ...
  public create(
    message: string,
    inputTrinary: boolean = false
  ): { payload: string; root: string; address: string } {
    ...
    const mam = Mam.createMessage(
      this.seed,
      TrytesMsg,
      this.channel.mode != MAM_MODE.PUBLIC ? this.channel.side_key : undefined,
      this.channel
    );
  ...
  public async attach(
    ...
    return new Promise<Transaction[]>((resolve, reject) => {
      ...
      const sendTrytes: any = createSendTrytes(
        this.provider,
        this.attachFunction
      );
  ...
  public EnablePowSrv(
    ...
    apiKey?: string,
    timeout: number = 3000,
    apiServer: string = 'https://api.powsrv.io:443'
  ) {
    if (enable && apiKey) {
      this.attachFunction = CreateAttachToTangleWithPwrSvr(
        apiKey,
        timeout,
        apiServer
      );
  ...
  public async catchUpThroughNetwork(): Promise<string[]> {
    return new Promise<string[]>(async (resolve, reject) => {
      ...
      while (!consumedAll) {
        ...
        if (
          this.channel.mode == MAM_MODE.PRIVATE ||
          this.channel.mode == MAM_MODE.RESTRICTED
        ) {
        ...
        await findTransactions({ addresses: [address] })
          .then(transactionHashes => {
            ...
            if (transactionHashes.length == 0) {
              ...
              const next_root_merkle = MamDetails.iota_merkle_create(
                MamDetails.string_to_ctrits_trits(this.seed),
                this.channel.start + this.channel.count,
                this.channel.next_count,
                this.channel.security
              );
              const next_root = MamDetails.iota_merkle_slice(next_root_merkle);
